...
awo::c_entity* get_entity( int index ) { // just leave it like that
	awo::c_entity* ent = ( awo::c_entity* )( awo::_address->get_entity_list_entry() + 0x78 * ( index + 1 ) );
	return ent;
}

void awo::hacks_t::run( ) { 
	/* update matrix */
	if ( !_proc_manager.read_memory( awo::_address->get_matrix_address( ), awo::_address->view.matrix, 64 ) ) {
#ifdef read_data_dbg
	...
	awo::_address->update_entity_list_entry( );

	/* local player stuff */
	DWORD64 local_player_adr = 0;
	DWORD64 local_pawn_adr = 0;
	if ( !_proc_manager.read_memory( awo::_address->get_local_controller_address( ), local_player_adr ) ) {
#ifdef read_data_dbg
	...
	if ( !_proc_manager.read_memory( awo::_address->get_local_pawn_address( ), local_pawn_adr ) ) {
#ifdef read_data_dbg
	...
	for ( int i = 0; i < 64; i++ ) {
		...
		if ( !_proc_manager.read_memory<DWORD64>( awo::_address->get_entity_list_entry( ) + ( i + 1 ) * 0x78, entity_address ) ) {
		...
		awo::col_t col = awo::col_t( 255, 0, 0 );
...
#define int_3
		...
		ImVec4 rect = awo::_esp->get_player_bounding_box( entity );
		awo::_esp->render_esp( local_player, entity, rect, local_player_index, i );
		awo::_esp->killed_by_hs( entity, i );
	}
	...
	for ( int i_smoke = 64; i_smoke < 1024; i_smoke++ ) {
		...
		awo::_esp->change_smoke_color( ent );
		awo::_esp->remove_smoke( ent );
	}
