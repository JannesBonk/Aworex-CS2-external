...
void awo::legit_t::run_aimbot( const c_entity& entity, const c_entity& local, vec3_t local_pos, int ent_idx, int local_idx ) {
...
void awo::legit_t::draw_aimbot_fov( ) { 
    ...
    _render->add_circle( awo::vec2_t( screen_x, screen_y ), ( ( _settings->fov + this->dinamic_csale * animation.value ) * 10 ), _render->to_main_color( _settings->fov_color ) );
}
