...
void awo::shots_t::hitsound( const c_entity& entity ) {
...
*/
void awo::shots_t::hitmarker( const c_entity& entity, const c_entity& local ) {
	if ( !_settings->hitmarker ) {
	...
	auto centerx = ImGui::GetIO().DisplaySize.x * 0.5;
	auto centery = ImGui::GetIO().DisplaySize.y * 0.5;
	...
	for ( int i = 0; i < this->hitmarkers.size( ); i++ ) {
		...
		// awo::_render->add_line( );
		awo::col_t col = this->hitmarkers[ i ].headshot ? awo::col_t( 255, 0, 0 ) : _render->to_main_color( _settings->hitmarker_col );

		awo::_render->add_line( awo::vec2_t( centerx - 8, centery - 8 ), awo::vec2_t(centerx - 4, centery - 4 ), col.modify_alpha( 255 * animation.value ), 1 );
		awo::_render->add_line( awo::vec2_t(centerx - 8, centery + 8 ), awo::vec2_t(centerx - 4, centery + 4 ), col.modify_alpha( 255 * animation.value ), 1 );
		awo::_render->add_line( awo::vec2_t(centerx + 8, centery - 8 ), awo::vec2_t(centerx + 4, centery - 4 ), col.modify_alpha( 255 * animation.value ), 1 );
		awo::_render->add_line( awo::vec2_t(centerx + 8, centery + 8 ), awo::vec2_t(centerx + 4, centery + 4 ), col.modify_alpha( 255 * animation.value ), 1 );
