...
void render_filled_3d_circle( const awo::vec3_t& origin, float radius, awo::col_t color ) {
	static constexpr float pi = 3.14159265358979323846f;

	static constexpr float Step = pi * 2.0f / 60;
	std::vector<ImVec2> points;
	for ( float lat = 0.f; lat <= pi * 2.0f; lat += Step ) {
		const auto& point3d = awo::vec3_t( sin( lat ), cos( lat ), 0.f ) * radius;
		awo::vec2_t point2d;
		if ( awo::_address->view.world_to_screen( origin + point3d, point2d ) )
...
/* rendering stuff gonna sit there */
void render_3d_circle( const awo::vec3_t& origin, float radius, awo::col_t color ) {
	static constexpr float pi = 3.14159265358979323846f;
	static constexpr float Step = pi * 2.0f / 60;
...
void awo::sound_info::push_sound( const c_entity& entity ) {
...
void awo::sound_info::push_sound2( const c_entity& entity, int idx ) { /* use that for enemy */
