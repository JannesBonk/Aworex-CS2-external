...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, col, false, 3);
    window->DrawList->AddRect( bb.Min, bb.Max, to_vec4(38, 38, 44, style.Alpha), 3 );
...
bool ImGui::Hotkey( const char* label, int* k ) {
    ...
    const ImVec2 size = ImVec2( CalcTextSize( KeyNames[*k] ).x + 10, 20 );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = GetWindowWidth( ) - 25;

    const ImRect clickable( window->DC.CursorPos + ImVec2( 0, 17 ), window->DC.CursorPos + ImVec2( size, 45 ) );
    ...
    window->DrawList->AddRectFilled( clickable.Min, clickable.Max, to_vec4(18, 18, 22, style.Alpha ), 3 );
    window->DrawList->AddRect( clickable.Min, clickable.Max, to_vec4(38, 38, 44, style.Alpha), 3 );

    RenderText( total_bb.Min, label );

    RenderTextClipped( clickable.Min + ImVec2( 9, 6 ), clickable.Max - ImVec2( 20, -6 + -CalcTextSize( preview_value, 0, 1 ).y ), preview_value, NULL, NULL, ImVec2( 0.f, 0.f ) );

    RenderArrow( window->DrawList, ImVec2( clickable.Max.x - 15, clickable.Min.y + 7 ), GetColorU32( ImGuiCol_Text, 0.7f ), ImGuiDir_Down, 12 );
...
void ImGui::MultiCombo(const char* label, bool combos[], const char* items[], int items_count)
{
    ...
    if (BeginCombo(label, preview.c_str()))
    {
        PushStyleVar( ImGuiStyleVar_ItemSpacing, ImVec2( 0, 0 ) );
        for (int i = 0; i < items_count; i++) {
            Selectable( items[i], &combos[i], ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_NoPadWithHalfSpacing, ImVec2( 0, 30 ) );
        }
        PopStyleVar( );
        EndCombo();
    }
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    const ImVec2 label_size = CalcTextSize( label, 0, 1 );
    const ImRect frame_bb( window->DC.CursorPos + ImVec2( 0, label_size.y + 4 ), window->DC.CursorPos + ImVec2( w, label_size.y + 10 ) );
    const ImRect total_bb( window->DC.CursorPos, frame_bb.Max );
    ...
    float percent = ( grab_bb.Max.x - window->DC.CursorPos.x - frame_bb.Min.x + window->Pos.x + 13 ) / frame_bb.GetWidth( );

    value->second = ImLerp( value->second, percent * frame_bb.GetWidth( ), 0.125f );
    // 196, 196, 196
    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, to_vec4(18, 18, 22, style.Alpha ), 5 );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.05f * style.Alpha ), 5 );
    window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( frame_bb.Min.x + value->second, frame_bb.Max.y ), custom.get_accent_color( style.Alpha ), 5 );
    window->DrawList->AddCircleFilled( ImVec2( frame_bb.Min.x + value->second - 2, grab_bb.Min.y + 1 ), 6, to_vec4(222, 222, 222, style.Alpha));
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(GetWindowWidth( ) - 25, 25), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 4), window->DC.CursorPos + frame_size + ImVec2(0, label_size.y + 4));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max);
    ...
    value->second = ImLerp( value->second, ( IsItemActive( ) ? 1.f : 0.f ), 0.045f );

    // Render frame
    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame( frame_bb.Min, frame_bb.Max, to_vec4(18, 18, 22, style.Alpha ), false, 3 );
        window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, to_vec4(38, 38, 44,  style.Alpha ), 3 );
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2( frame_bb.Min.x + style.FramePadding.x, frame_bb.GetCenter( ).y - CalcTextSize( buf ).y / 2 );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2( 22, 12 )))
        {
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
    ...
    draw_list->AddRectFilled( pos + ImVec2( 1, 1 ), pos + ImVec2( bar_w + 1, 5 ), ImColor( 0.f, 0.f, 0.f, 0.2f ), 4 );
    draw_list->AddRectFilled( pos, pos + ImVec2( bar_w, 4 ), ImColor( 1.f, 1.f, 1.f ), 4 );
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_width = square_sz - 8; // Arbitrary smallish width of Hue/Alpha picking bars
    ...
    custom.colored_rect( std::string( label ).append( ".colored_rect" ).c_str( ), ImColor( col[0], col[1], col[2] ), ImVec2( square_sz * 8.1f, 25 ) );
    ...
    char rgba_buf[64];
    if ( flags & ImGuiColorEditFlags_NoAlpha )
        ImFormatString( rgba_buf, IM_ARRAYSIZE( rgba_buf ), "%.0f, %.0f, %.0f", col[0] * 255, col[1] * 255, col[2] * 255 );
    else
        ImFormatString( rgba_buf, IM_ARRAYSIZE( rgba_buf ), "%.0f, %.0f, %.0f, %.0f", col[0] * 255, col[1] * 255, col[2] * 255, col[3] * 255 );
       
    custom.picker_widget( std::string( "HEX" ).append( "##" ).append( label ).c_str( ), hex_buf, ImVec2( square_sz * 8.1f, 25 ) );
    custom.picker_widget( std::string( flags & ImGuiColorEditFlags_NoAlpha ? "RGB" : "RGBA" ).append( "##" ).append( label ).c_str( ), rgba_buf, ImVec2( square_sz * 8.1f, 25 ) );

    if ( custom.button( "Copy", "M", ImVec2( square_sz * 3.85f, 30 ) ) )
        memcpy( custom.col_buf, col, components * sizeof( float ) );

    SameLine( );

    if ( custom.button( "Paste", "N", ImVec2( square_sz * 3.85f, 30 ) ) ) {
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(18, 18, 22), col_white, hue_color32, hue_color32, col_white, 4);
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(18, 18, 22), 0, 0, col_black, col_black, 4);
        ...
        sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);
         ...
         // Render Hue Bar
        draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + 0 * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (0 + 1) * (sv_picker_size / 6)), ImColor(18, 18, 22), col_hues[0], col_hues[0], col_hues[0 + 1], col_hues[0 + 1], 4, ImDrawFlags_RoundCornersTop);

        for (int i = 1; i < 5; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);

        draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + 5 * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (5 + 1) * (sv_picker_size / 6)), ImColor(18, 18, 22), col_hues[5], col_hues[5], col_hues[5 + 1], col_hues[5 + 1], 4, ImDrawFlags_RoundCornersBottom);

        float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
        RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 4);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, ImColor(18, 18, 22), user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 4);
        float bar1_line_y = IM_ROUND(picker_pos.y + (1.0f - alpha) * sv_picker_size);
        RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 4);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2( 0, 1 ), window->DC.CursorPos + size + ImVec2( 0, 1 ));
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32( ImGuiCol_Text, 0.1f ), rounding); // Color button are often in need of some sort of border
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        // FIXME: We can awolve this into user accessible helpers to add extra buttons on title bars, headers, etc.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderFrame( bb.Min, bb.Max, ImColor( 1.f, 1.f, 1.f, 0.1f * value->second ), 0, 5 );

    if ( selected || hovered )
        PushFont( GetIO( ).Fonts->Fonts[1] );

    PushStyleColor( ImGuiCol_Text, ImVec4( 1.f, 1.f, 1.f, ( selected ? 1.f : 0.5f ) ) );
    RenderText( ImVec2( bb.Min.x + 9, bb.GetCenter( ).y - label_size.y / 2 ), label );
    PopStyleColor( );
