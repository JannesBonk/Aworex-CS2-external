...
void evo::menu_t::render() {
    ...
    alpha = ImLerp(alpha, evo::framework::m_b_open ? 1.f : 0.f, 1.f * speed);
    ...
    if (evo::framework::m_b_open || alpha >= 0.05f) {
                ...
                case 1:
                {
                    ...
                    BeginChild("##ragebot.groupboxes.area", ImVec2(size.x - 20, size.y - 90)); {
                        ...
                        custom.group_box_alternative("##master_switch.ragebot", ImVec2(GetWindowWidth(), 53)); {
                            ...
                            custom.checkbox("Master switch##ragebot", &evo::_settings->rage, "Toggle on all ragebot features", GetIO().Fonts->Fonts[1]);

                        } custom.end_group_box_alternative();

                        custom.group_box("General", ImVec2(GetWindowWidth() / 2 - GetStyle().ItemSpacing.x / 2, GetWindowHeight() - 53 - GetStyle().ItemSpacing.y)); {
                            ImGui::Checkbox("Ignore wall", &evo::_settings->ignore_wall);
            ...
            case 1:
            {
                ...
                BeginChild("##visuals.area", ImVec2(size.x - 20, size.y - 90)); {
                    ...
                    custom.group_box_alternative("##master_switch.visuals", ImVec2(GetWindowWidth(), 53)); {
                        ...
                        custom.checkbox("Master switch##visuals", &evo::_settings->visuals, "Toggle on all visual features", GetIO().Fonts->Fonts[1]);
                    ...
                    custom.group_box("General", ImVec2(GetWindowWidth() / 2 - GetStyle().ItemSpacing.x / 2, GetWindowHeight() - 53 - GetStyle().ItemSpacing.y)); {
                        ...
                        if (evo::_settings->eap) {
                            ImGui::Checkbox("Weaon name animation", &evo::_settings->weapon_animation);
                            ImGui::SameLine(GetWindowWidth() - 58);
                            ImGui::ColorEdit4("###nameanimcl", evo::_settings->weapon_color_a, ALPHA);

                            ImGui::Combo("Weaon name animation type", &evo::_settings->weapon_at, animation_types.data(), animation_types.size());
