...
namespace evo {
	...
	public:
		...
		void bind_animation( int id, std::string text, evo::col_t color, ImFont* font, int x, int y, text_animation_t animation_type, float animation_speed );

		void clip_rect( float x, float y, float w, float h, std::function<void( )> function );

		evo::vec2_t text_size( const char* text, ImFont* font );
		ImFont* font_tahoma;
