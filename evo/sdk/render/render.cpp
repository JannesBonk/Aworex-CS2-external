...
void evo::render_t::add_text( int x, int y, col_t c, ImFont* font, const char* text, int flag ) {
...
evo::vec2_t evo::render_t::text_size( const char* text, ImFont* font ) {
	ImGuiIO& io = ImGui::GetIO( );
...
void evo::render_t::bind_animation( int id, std::string text, evo::col_t color, ImFont* font, int x, int y, text_animation_t animation_type, float animation_speed ) {
