...
void evo::esp_t::render_name( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) { 
	...
	/* text sizes */
	int text_width = evo::_render->font_tahoma->CalcTextSizeA(15.f, FLT_MAX, NULL, player_name.c_str()).x;
	int text_height = evo::_render->font_tahoma->CalcTextSizeA(15.f, FLT_MAX, NULL, player_name.c_str()).y;
	
	
	evo::_render->add_text(
		rect.x + ( rect.z  * 0.5f ) - ( text_width * 0.5f ), rect.y - text_height - 2, name_color, evo::_render->font_tahoma, 
		player_name.c_str( ), evo::font_flags_t::outline
	);
	...
	if (evo::_settings->name_animation) {
		evo::_render->bind_animation(index, player_name.c_str(), _render->to_main_color(_settings->name_color_a).modify_alpha(this->esp_alpha[index]),
			evo::_render->font_tahoma, rect.x + (rect.z * 0.5f) - (text_width * 0.5f), rect.y - text_height - 2, (evo::text_animation_t)evo::_settings->name_at, 0.1);
	}
...
void evo::esp_t::health_bar( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) { 
	...
	if ( player_hp < 98 ) {
		evo::_render->add_text( rect.x - ( ( player_hp > 10 ) ? 8 : 7 ),
								rect.y + 1 + ( ( h - fill ) - evo::_render->font_small_pixel->FontSize), evo::col_t().modify_alpha( this->esp_alpha[ index ] ), evo::_render->font_small_pixel, std::to_string( player_hp ).c_str(), evo::font_flags_t::outline);
	}
}
	...
	if ( max != -1 ) {
		...
		if ( bullets < ( max - 2 ) ) {
			...
			evo::_render->add_text( rect.x - 1 + bar,
									rect.y - 3 + rect.w + 2, evo::col_t( ).modify_alpha( this->esp_alpha[ index ] ), evo::_render->font_small_pixel, std::to_string( bullets ).c_str( ), evo::font_flags_t::outline );
		}
	}
}
...
void evo::esp_t::render_weapon( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) { 
	...
	/* text sizes */
	int text_width = evo::_render->text_size( weaepon_name.c_str( ), evo::_render->font_tahoma).x;
	int text_height = evo::_render->text_size( weaepon_name.c_str( ), evo::_render->font_tahoma).y;
	...
	evo::_render->add_text( rect.x + ( rect.z * 0.5f ) - ( text_width * 0.5f ), rect.y + rect.w + 1 + offset + ( _settings->visuals_b[ 0 ] ? text_height : 0 ), weapon_clr,
		evo::_render->font_tahoma, weaepon_name.c_str( ), evo::font_flags_t::outline);

	if (evo::_settings->weapon_animation) {
		evo::_render->bind_animation(index, weaepon_name.c_str(), _render->to_main_color(_settings->weapon_color_a).modify_alpha(this->esp_alpha[index]),
			evo::_render->font_tahoma, rect.x + (rect.z * 0.5f) - (text_width * 0.5f), rect.y + rect.w + 1 + offset + (_settings->visuals_b[0] ? text_height : 0), (evo::text_animation_t)evo::_settings->weapon_at, 0.1);
	}
...
void evo::esp_t::render_side_info( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) {
	...
	if ( evo::_settings->show_competivie_wins ) {
		info.push_back( { "cw: " + std::to_string( entity.controller.wins ), evo::col_t( ) } ); // i guess it works
	}
#endif
	...
	for ( int i = 0; i < info.size( ); i++ ) {
		...
		evo::_render->add_text( rect.x + rect.z + 6, rect.y + 1 + 
								( ( evo::_render->text_size( info[ i ].inf.c_str( ), evo::_render->font_small_pixel).y + 1 ) ) * i, info[ i ].col.modify_alpha( this->esp_alpha[ index ] ), evo::_render->font_small_pixel, info[ i ].inf.c_str( ), evo::font_flags_t::outline );
	}
}
...
void evo::esp_t::render_dropped_esp( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) { /* shit doesnt work */
	...
	for ( int i = 65; i < 1024; i++ ) {
		...
		evo::_render->add_text( entity.player_pawn.vec_origin.x, entity.player_pawn.vec_origin.y, evo::col_t( ), evo::_render->font_tahoma, name.c_str( ) );
	}
}
...
void evo::esp_t::render_distance( const c_entity& local_player, const c_entity& entity, ImVec4 rect, int local_index, int index ) {
	...
	/* text sizes */
	int text_width = evo::_render->text_size( dist.c_str( ), evo::_render->font_tahoma).x;
	int text_height = evo::_render->text_size( dist.c_str( ), evo::_render->font_tahoma).y;
	...
	evo::_render->add_text( rect.x + ( rect.z * 0.5f ) - ( text_width * 0.5f ), rect.y + offset + rect.w + 1, clr,
		evo::_render->font_tahoma, dist.c_str( ), evo::font_flags_t::outline);
}
...
void evo::esp_t::killed_by_hs( const c_entity& entity, int i ) {
	...
	evo::_render->add_text( head.screen_pos.x, head.screen_pos.y,
							evo::col_t( 0, 255, 0 ).modify_alpha( 255 * animation.value ), evo::_render->font_tahoma, hs.c_str( ) );
}
